...
public class Spawner : Singleton<Spawner> {
    ...
    IEnumerator SpawnCycle()
    {
        ...
        while (true)
        {
            ...
            if (Up.Length > 2)
            {
                GameObject go = AssetManager.Instance.GetOne(Up);
                go.transform.position = UpLine.position;
            }

            if (Down.Length > 2)
            {
                GameObject go = AssetManager.Instance.GetOne(Down);
                go.transform.position = DownLine.position;
            }

            if (Left.Length > 2)
            {
                GameObject go = AssetManager.Instance.GetOne(Left);
                go.transform.position = LeftLine.position;
            }

            if (Right.Length > 2)
            {
                GameObject go = AssetManager.Instance.GetOne(Right);
